...
async fn test_memory_eviction() {
    ...
    assert!(history.len() <= max, "history len {} exceeds max {}", history.len(), max);
}
...
async fn test_shell_tool_with_allowlist() {
    ...
    let result = agent.execute_tool("shell", json!({"bin":"echo","argv":["hello"]})).await.unwrap();
...
async fn test_shell_tool_not_allowlisted() {
    ...
    let result = agent.execute_tool("shell", json!({"bin":"false","argv":[]})).await;
...
async fn test_web_get_tool_basic() {
    ...
    let result = agent.execute_tool("web.get", json!({"url":"https://httpbin.org/status/200"})).await;
