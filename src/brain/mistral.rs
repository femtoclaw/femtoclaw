...
impl MistralBrain {
    pub fn from_env() -> anyhow::Result<Self> {
        let api_key = std::env::var("FEMTO_MISTRAL_API_KEY").map_err(|_| {
            anyhow::anyhow!("FEMTO_MISTRAL_API_KEY is required when FEMTO_BRAIN=mistral")
        })?;
        let model = std::env::var("FEMTO_MISTRAL_MODEL")
...
impl Brain for MistralBrain {
    async fn think(&self, messages: &[Message]) -> anyhow::Result<String> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(anyhow::anyhow!("mistral request failed: {status} {text}"));
        }

        let v: serde_json::Value = resp.json().await?;
        let content = v["choices"][0]["message"]["content"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("unexpected mistral response shape"))?;
